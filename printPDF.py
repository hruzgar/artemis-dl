...
def remove_unnecessary_elements(soup):
    ...
    soup = remove_with_selector_if_exists(soup, element_paths.exercise_results_row_1)
    soup = remove_with_selector_if_exists(soup, element_paths.exercise_results_row_2)
    soup = remove_with_selector_if_exists(soup, element_paths.exercise_results_row_3)
    ...
    soup = remove_with_selector_if_exists(soup, element_paths.exercise_assessment_text)
    ...
    list = soup.css.select(element_paths.exercise_community_field)
    if len(list) != 0:
        list[0].decompose()
    ###
    soup.css.select(element_paths.exercise_footer)[0].decompose() # Footer (About, Privacy und so ganz unten)
    return soup
